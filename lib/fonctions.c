...
#include "prototypes_and_enums.h"
    ...
    if(turn_to_attack == 1){
        ...
        if(strcmp(Attacker->name , "Lifeline") == 0 || strcmp(Attacker->name , "The_Medic") == 0){
            ...
            for (int i=0 ; i < aly_count ; i++){
                if(x_click >= aly_pos[i]->x && x_click <= (aly_pos[i]-> x + aly_pos[i]-> w) && y_click >= aly_pos[i]->y && y_click <= (aly_pos[i]-> y + aly_pos[i] -> h ) && if_still_alive_only(aly_team[i])){
