...
void special_attack(Fighter* Attacker, Fighter* Target) {
    if (strcmp(Attacker->name, "John_Wick") == 0) {
        int damage = (int)(Attacker->attack * 0.75);  // 75% damage per shot
        Target->hp -= 2 * damage;
    } else if (strcmp(Attacker->name, "Hulk") == 0) {
        Target->attack = (int)(Target->attack * 0.8);  // Reduces attack by 20%
        ...
        Target->defense = (int)(Target->defense * 0.7);  // Reduces defense by 30%
    } else if (strcmp(Attacker->name, "Dracula") == 0) {
        int heal = (int)(Attacker->attack * 0.4);  // Heal 40% of damage dealt
        Target->hp -= heal;
